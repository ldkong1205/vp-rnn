...
global mu_p mu_v qP qM qDp qDm qDDp qDDm myInf n m Pinfty Minfty gamma beta Time;
n=6;                                    %??????????????6??????
m=3;                                    %??????????????????????????????3??????????
gamma=80;   
K=1*eye(m,m);
...
tic;   %????????
N=1;
[t,y]=ode15s(@mvn_ZZJLJrebi1_net,[0,N*T],init,options); %ode15s much better than ode45 KLD %ODE????
cpu_time=toc                                                        %????????
size(t)
size(y)
